...
class PatchDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        distMap = distMap.astype(np.float32) # Back to float 32.

        bmap = torch.from_numpy(bmap).unsqueeze(0) # Changes dimensions from H, W, C to C, H, W
        distMap = torch.from_numpy(distMap).unsqueeze(0) # Changes dimensions from H, W, C to C, H, W
        img = torch.from_numpy(img).permute(2, 0, 1) # Changes dimensions from H, W, C to C, H, W
